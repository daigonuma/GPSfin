...
class GPSService: Service(), LocationListener {
     ...
     * URL は、環境に合わせて変更すること
     */
    private val urlString = "http://10.111.136.184:5000/gps/register?longitude=%f&latitude=%f&bus=%d&stop=%f"
        ...
        if (http) {
            ...
            class HttpRunnable: Runnable {
                override fun run() {
                    ...
                    // URL文字列の生成
                    val registerUrl =
                        String.format(urlString, location.longitude, location.latitude, bus, stop)
                    //val registerUrl = "http://10.111.136.184:5000/gps/register?longitude=%f&latitude=%f&bus=%d&"+"stop=${stop}"
